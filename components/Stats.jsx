...
function Counter({ value }) {
  ...
  // This hook detects if the element is on the screen
  const isInView = useInView(ref, { once: true, margin: "-50px" });
  
  const spring = useSpring(0, { 
    mass: 1, 
    stiffness: 60, // Slower, smoother climb
    damping: 20 
  });
  ...
  useEffect(() => {
    if (isInView) {
      // Only start the spring animation when the user scrolls here
      spring.set(value);
    }
      ...
      <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-3 gap-12 text-center">
        {items.map((item, i) => (
          <div key={i} className="flex flex-col items-center">
            ...
            <p className="text-gray-400 uppercase tracking-widest text-[10px] md:text-xs font-bold">
              {item.label}
            </p>
          </div>
        ))}
